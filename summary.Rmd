...
COVID-19 numbers of cases and death rate?**
...
For these two box-plot diagrams, the Total Cases infected are higher if the Stay at Home Policy is ordered After the first cases. Thus, we can say the Stay at Home Policy does decrease the Total Cases infected if it is ordered before the First Case occurred. 
...
It is difficult to determine whether the two variables are related from the figure.
...
```{r,disease6,echo=F}
#disease6<-disease5[ , -which(colnames(disease5) %in% c("total_cases"))]
...
lm.dis5<-lm(deaths~sleep_hour+sleep_hour_high+Low_birthweight+heart_disease+adult_obesity+Food_environment+Respiratory+liver_Total_death,data = disease5)
summary(lm.dis5)
faraway::vif(lm.dis5)
